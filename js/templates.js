  ...
  if (currentTemplateTab === 'training') {
    ...
    container.innerHTML = `
      ...
      ${templates?.length ? templates.map(t => {
        ...
        const sessionTags = sd.map(s => `<span style="display:inline-block;padding:3px 10px;background:var(--bg3);border-radius:6px;font-size:11px;color:var(--text2);font-weight:500;">${escHtml(s.nom || 'Séance')}</span>`).join(' ');
        return `
        <div class="card" style="margin-bottom:16px;cursor:pointer;" onclick="editWorkoutTemplate('${t.id}')">
          <div class="card-header">
            <div style="flex:1;">
              <div class="card-title">${escHtml(t.nom)}</div>
              <div style="color:var(--text3);font-size:12px;margin-top:4px;">${sd.length} séance(s) · ${totalEx} exercices · ${totalSeries} séries</div>
              ${sessionTags ? `<div style="display:flex;flex-wrap:wrap;gap:4px;margin-top:8px;">${sessionTags}</div>` : ''}
            </div>
            <div style="display:flex;gap:8px;" onclick="event.stopPropagation()">
              <button class="btn btn-outline btn-sm" onclick="editWorkoutTemplate('${t.id}')"><i class="fas fa-pen"></i></button>
              <button class="btn btn-outline btn-sm btn-danger" onclick="deleteWorkoutTemplate('${t.id}')"><i class="fas fa-trash"></i></button>
            </div>
          </div>
        </div>`;
      }).join('') : '<div class="empty-state"><i class="fas fa-dumbbell"></i><p>Aucun template training</p></div>'}
    `;
  } else {
    ...
    // Filter by template_type (legacy templates without type → 'jour')
    ...
    const emptyLabel = ntplSub === 'diete' ? 'Aucun template diète' : ntplSub === 'jour' ? 'Aucun template jour' : 'Aucun template repas';
    ...
    container.innerHTML = `
      ...
      ${filtered.length ? filtered.map(t => {
        ...
        const subtitle = ntplSub === 'diete'
          ? `${t.calories_objectif||0} kcal · P:${t.proteines||0}g G:${t.glucides||0}g L:${t.lipides||0}g`
          : ntplSub === 'jour'
        ...
        <div class="card" style="margin-bottom:16px;">
          <div class="card-header">
            <div>
              <div class="card-title">${escHtml(t.nom)}</div>
              <div style="color:var(--text2);font-size:12px;margin-top:4px;">${subtitle}</div>
            </div>
      ...
      }).join('') : `<div class="empty-state"><i class="fas fa-utensils"></i><p>${emptyLabel}</p></div>`}
    `;
  }
}
...
function renderTplEditor(el, progName, patternType, pd, isEdit) {
  // Reuses the same Synergy layout as program editor but targets templates-content
