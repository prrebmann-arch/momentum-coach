...
export async function POST(request: Request) {
  ...
  try {
    ...
    if (body.object === 'instagram' || body.object === 'page') {
      for (const entry of (body.entry || [])) {
        for (const event of (entry.messaging || [])) {
          ...
          let igAccount: { user_id: string; ig_user_id: string; page_id: string; page_access_token: string; access_token: string } | null = null;
          ...
          try {
            ...
            console.log('[ig-webhook] Name fetch failed:', (nameErr as Error).message);
          }
    ...
    console.error('[ig-webhook] Processing error:', (err as Error).message);
  }
