...
export async function POST(req: NextRequest) {
  ...
  if (uploaded_by === 'athlete') {
    ...
    if (!file_path.startsWith(`${user.id}/`)) return NextResponse.json({ error: 'invalid path for athlete' }, { status: 400 })
  } else {
    if (ath.coach_id !== user.id) return NextResponse.json({ error: 'forbidden' }, { status: 403 })
    if (!file_path.startsWith(`coach/${user.id}/${athlete_id}/`)) return NextResponse.json({ error: 'invalid path for coach' }, { status: 400 })
  }
