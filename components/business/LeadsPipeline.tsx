...
export default function LeadsPipeline() {
  ...
  const loadLeads = useCallback(async () => {
    ...
    setLoading(true)
    try {
      const { data } = await supabase.from('leads').select('id, user_id, name, instagram_handle, email, phone, source, status, notes, tags, message_count, created_at').eq('user_id', user.id).order('created_at', { ascending: false }).limit(200)
